...
it('sends notifications to all project members when a task is created for a project', function () {
    Notification::fake();
    ...
    Notification::assertSentTo(
        $this->projectManager,
        TaskCreated::class,
        fn ($notification) => $notification->task->title === 'New Task for Project'
...
it('does not send notification to the user who created the task', function () {
    Notification::fake();
...
it('does not send notifications when task is not associated with a project', function () {
    Notification::fake();
...
it('does not send notifications when task is associated with a program', function () {
    Notification::fake();
    ...
    $taskData = [
        ...
        'priority' => 'medium',
        'status_id' => $this->status->id,
        'taskable_type' => 'App\\Models\\Program',
...
it('sends notification only once when user has multiple roles in project', function () {
    Notification::fake();
...
it('sends notifications to members added via members relationship', function () {
    Notification::fake();
...
it('handles project without manager or reviewer gracefully', function () {
    Notification::fake();
...
it('does not send notifications when project has no members', function () {
    Notification::fake();
...
it('notification mail has correct subject', function () {
    $task = Task::factory()->create([
        'title' => 'Test Task',
        'status_id' => $this->status->id,
        'taskable_type' => 'App\\Models\\Project',
...
it('notification uses both mail and database channels', function () {
    $task = Task::factory()->create([
        'title' => 'Test Task',
        'status_id' => $this->status->id,
    ]);
...
it('notification shows correct priority labels', function () {
    $priorities = [
        'lowest' => 'Très basse',
        'low' => 'Basse',
        'medium' => 'Moyenne',
...
it('notification works without creator (system notification)', function () {
    $task = Task::factory()->create([
        'title' => 'System Task',
        'status_id' => $this->status->id,
    ]);

    $notification = new TaskCreated($task, $this->project, null);
    $dbData = $notification->toDatabase($this->participant1);
...
it('notification includes assignee info when task is assigned', function () {
...
it('getAllNotifiableUsers returns all project users', function () {
...
it('getAllNotifiableUsers excludes specified user', function () {
...
it('getAllNotifiableUsers removes duplicates', function () {
    ...
    $managerCount = $users->filter(fn ($user) => $user->id === $this->projectManager->id)->count();
    expect($managerCount)->toBe(1);
});

it('getAllNotifiableUsers includes members from project_members table', function () {
    $member = User::factory()->create();
...
it('creates task and sends notification in same transaction', function () {
    Notification::fake();
    ...
    Notification::assertSentTo(
        $this->projectManager,
        TaskCreated::class,
        fn ($notification) => $notification->task->title === 'Transaction Task'
    );
});

it('uses project_id backward compatibility for notifications', function () {
    Notification::fake();
...
it('redirects to project when from_project flag is set', function () {
    Notification::fake();
