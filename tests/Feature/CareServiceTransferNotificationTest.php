...
it('sends email to new pastor when appointment is transferred', function (): void {
    ...
    $this->actingAs($this->admin)
        ->post(route('care-service.transfer', $this->appointment), [
            'transferred_to_id' => $this->newPastor->id,
            'transfer_reason' => 'Changement de planning',
        ])
        ->assertRedirect();

    Mail::assertQueued(CareServiceTransferNotification::class, fn ($mail): bool => $mail->hasTo($this->newPastor->email)
...
it('sends email to old pastor when appointment is transferred', function (): void {
    ...
    Mail::assertQueued(CareServiceTransferNotification::class, fn ($mail): bool => $mail->hasTo($this->oldPastor->email)
...
it('sends email to client when appointment is transferred', function (): void {
    ...
    Mail::assertQueued(CareServiceTransferNotification::class, fn ($mail): bool => $mail->hasTo($this->appointment->client_email)
...
it('sends all three emails when appointment is transferred', function (): void {
    ...
    Mail::assertQueued(CareServiceTransferNotification::class, 3);
});
...
it('sends email to client even when they do not have an account', function (): void {
    ...
    $externalAppointment = CareService::factory()->create([
        'pastor_id' => $this->oldPastor->id,
        'user_id' => null,
    ...
    $this->actingAs($this->admin)
        ->post(route('care-service.transfer', $externalAppointment), [
            'transferred_to_id' => $this->newPastor->id,
        ])
        ->assertRedirect();

    // Email should still be queued for client
    Mail::assertQueued(CareServiceTransferNotification::class, fn ($mail): bool => $mail->hasTo('external@example.com')
        && $mail->recipientType === 'client');
});
...
it('includes transfer reason in notifications when provided', function (): void {
    ...
    Mail::assertQueued(CareServiceTransferNotification::class, fn ($mail): bool => $mail->appointment->transfer_reason === $transferReason);
...
it('mailable has correct subject for new pastor', function (): void {
    $mailable = new CareServiceTransferNotification(
        $this->appointment,
        $this->oldPastor,
        $this->newPastor,
        'new_pastor'
...
it('requires transfer care service permission to transfer', function (): void {
    Mail::fake();
