...
class CareServiceAvailabilitySelectedSlotsTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function pastor_can_create_availability_with_selected_slots(): void
    {
        ...
        $response = $this->post(route('care-service-availability.store'), $data);

        $response->assertStatus(302);
        $response->assertRedirect(route('care-service-availability.index'));

        // Verify the availability was created with selected slots
        $availability = CareServiceAvailability::where('pastor_id', $this->pastor->id)
            ->where('day_of_week', 1)
            ->first();
    ...
    public function pastor_can_update_selected_slots(): void
    {
        ...
        $response = $this->put(route('care-service-availability.update', $availability->id), $updateData);
