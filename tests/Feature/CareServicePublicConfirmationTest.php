...
it('can confirm a pending appointment via public API without authentication', function (): void {
    $response = $this->postJson("/api/care-service/appointments/{$this->appointment->uuid}/confirm");
...
it('returns 404 when confirming non-existent appointment', function (): void {
    $response = $this->postJson('/api/care-service/appointments/non-existent-uuid/confirm');
...
it('can cancel a pending appointment via public API without authentication', function (): void {
    $response = $this->postJson("/api/care-service/appointments/{$this->appointment->uuid}/cancel", [
...
it('returns 404 when cancelling non-existent appointment', function (): void {
    $response = $this->postJson('/api/care-service/appointments/non-existent-uuid/cancel', [
...
it('cannot cancel appointment within 24 hours of appointment time', function (): void {
    ...
    $soonAppointment = CareService::create([
        'pastor_id' => $this->pastor->id,
    ...
    $response = $this->postJson("/api/care-service/appointments/{$soonAppointment->uuid}/cancel", [
        'cancellation_reason' => 'Test',
    ]);
...
it('can view appointment details via public API', function (): void {
    $response = $this->getJson("/api/care-service/appointments/{$this->appointment->uuid}");
...
it('returns 404 for confirmation status of non-existent appointment', function (): void {
    $response = $this->getJson('/api/care-service/appointments/fake-uuid/confirmation-status');
...
it('handles concurrent confirmation attempts gracefully', function (): void {
    ...
    $response1 = $this->postJson("/api/care-service/appointments/{$this->appointment->uuid}/confirm");
    $response1->assertStatus(200);

    $response2 = $this->postJson("/api/care-service/appointments/{$this->appointment->uuid}/confirm");
    $response2->assertStatus(400);
});

it('handles SQL injection attempts in UUID', function (): void {
    $maliciousUuid = "'; DROP TABLE care_services; --";

    $response = $this->getJson("/api/care-service/appointments/{$maliciousUuid}");

    $response->assertStatus(404);
});

it('returns proper error for empty UUID', function (): void {
    $response = $this->getJson('/api/care-service/appointments/');
...
it('can confirm zoom appointment', function (): void {
    $zoomAppointment = CareService::create([
        'pastor_id' => $this->pastor->id,
        'client_name' => 'Test Client',
        'client_email' => 'test@example.com',
    ...
    $response = $this->postJson("/api/care-service/appointments/{$zoomAppointment->uuid}/confirm");
...
it('can confirm hybrid appointment', function (): void {
    $hybridAppointment = CareService::create([
        'pastor_id' => $this->pastor->id,
        'client_name' => 'Test Client',
        'client_email' => 'test@example.com',
        'appointment_date' => Carbon::tomorrow(),
    ...
    $response = $this->postJson("/api/care-service/appointments/{$hybridAppointment->uuid}/confirm");
...
it('can confirm appointments with different durations', function ($duration): void {
    ...
    $response = $this->postJson("/api/care-service/appointments/{$appointment->uuid}/confirm");
