...
beforeEach(function (): void {
    ...
    $this->appointment = CareService::factory()->create([
...
test('confirmation tokens are unique per appointment', function (): void {
    $appointment1 = CareService::factory()->create(['pastor_id' => $this->pastor->id]);
    $appointment2 = CareService::factory()->create(['pastor_id' => $this->pastor->id]);
...
test('client can confirm appointment with valid token', function (): void {
    ...
    $response = $this->postJson('/api/care-service/confirm-by-client', [
...
test('client cannot confirm past appointment', function (): void {
    $pastAppointment = CareService::factory()->create([
        'pastor_id' => $this->pastor->id,
...
test('pastor can confirm appointment with valid token', function (): void {
    ...
    $response = $this->postJson('/api/care-service/confirm-by-pastor', [
...
test('appointment is confirmed when both parties confirm', function (): void {
    ...
    $this->postJson('/api/care-service/confirm-by-client', [
        'token' => $this->appointment->client_confirmation_token,
    ]);
...
test('order of confirmation does not matter', function (): void {
    ...
    $this->postJson('/api/care-service/confirm-by-pastor', [
        'token' => $this->appointment->pastor_confirmation_token,
    ]);
...
test('client confirmation is logged in activity', function (): void {
    ...
    $activity = Activity::where('subject_type', CareService::class)
...
test('partial confirmation sends notification to other party', function (): void {
    ...
    Mail::assertQueued(CareServicePartialConfirmationNotification::class, fn ($mail): bool => $mail->hasTo($this->pastor->email) && $mail->recipientType === 'pastor');
});
...
test('dual confirmation sends notifications to both parties', function (): void {
    ...
    Mail::assertQueued(CareServiceDualConfirmationNotification::class, fn ($mail) => $mail->hasTo($this->appointment->client_email));

    Mail::assertQueued(CareServiceDualConfirmationNotification::class, fn ($mail) => $mail->hasTo($this->pastor->email));
});
...
test('follow-up sends notifications to both client and pastor', function (): void {
    ...
    $this->actingAs($this->pastor)
        ->postJson("/api/care-service/appointments/{$this->appointment->uuid}/follow-up", [
    ...
    Mail::assertQueued(CareServiceFollowUpNotification::class, fn ($mail) => $mail->hasTo($this->appointment->client_email));

    Mail::assertQueued(CareServicePastorFollowUpNotification::class, fn ($mail) => $mail->hasTo($this->pastor->email));
});
...
test('follow-up creates appointment with confirmation tokens', function (): void {
    ...
    $followUp = CareService::where('parent_id', $this->appointment->id)->first();
...
test('can get confirmation status via API', function (): void {
    ...
    $response = $this->getJson("/api/care-service/appointments/{$this->appointment->uuid}/confirmation-status");
...
test('confirmation status returns 404 for invalid uuid', function (): void {
    $response = $this->getJson('/api/care-service/appointments/invalid-uuid/confirmation-status');
...
test('can find appointment by client token', function (): void {
    $found = CareService::findByClientToken($this->appointment->client_confirmation_token);
...
test('can find appointment by pastor token', function (): void {
    $found = CareService::findByPastorToken($this->appointment->pastor_confirmation_token);
...
test('returns null for non-existent token', function (): void {
    $found = CareService::findByClientToken('non_existent_token_12345678901234567890123456789012345678901234');
