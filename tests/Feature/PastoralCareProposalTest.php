...
beforeEach(function (): void {
    ...
    Role::firstOrCreate(['name' => 'mlr-agent']);
    ...
    $this->mlrAgent = User::factory()->create([
        'first_name' => 'Marie',
        'last_name' => 'MLR',
        'email' => 'mlr@example.com',
    ]);
    $this->mlrAgent->assignRole('mlr-agent');
...
it('can submit a proposal for a custom appointment date', function (): void {
    ...
    $response = $this->postJson('/api/pastoral-care/proposals', $proposalData);
...
it('can view a proposal by token', function (): void {
    ...
    $response = $this->getJson('/api/pastoral-care/proposals/show?token='.$proposal->proposal_token);
...
it('allows MLR agent to accept a proposal and assign a pastor', function (): void {
    ...
    $response = $this->actingAs($this->mlrAgent)
        ->postJson("/api/pastoral-care/proposals/{$proposal->uuid}/accept", [
            'pastor_id' => $this->pastor->id,
        ]);
    ...
    expect($proposal->mlr_agent_id)->toBe($this->mlrAgent->id);
...
it('allows MLR agent to reject a proposal without counter-proposal', function (): void {
    ...
    $response = $this->actingAs($this->mlrAgent)
        ->postJson("/api/pastoral-care/proposals/{$proposal->uuid}/reject", [
...
it('allows client to accept a counter-proposal', function (): void {
    ...
    $response = $this->postJson('/api/pastoral-care/proposals/accept-counter', [
...
it('allows client to reject a counter-proposal', function (): void {
    ...
    $response = $this->postJson('/api/pastoral-care/proposals/reject-counter', [
        'token' => $proposal->proposal_token,
...
it('correctly identifies proposals with counter-proposals', function (): void {
    ...
    $proposalWithCounter = PastoralCare::create([
        'pastor_id' => $this->pastor->id,
    ...
    // Simulate MLR sending counter-proposal
...
it('filters proposals using scopes', function (): void {
    ...
    expect(PastoralCare::proposed()->count())->toBe(5);
    expect(PastoralCare::isProposal()->count())->toBe(5);
    expect(PastoralCare::pendingProposals()->count())->toBe(3);
    expect(PastoralCare::counterProposed()->count())->toBe(2);
});
...
it('prevents accepting already processed proposal', function (): void {
    ...
    $proposal->acceptProposal($this->pastor->id, $this->mlrAgent->id);
