...
beforeEach(function (): void {
    ...
    $this->followUp = CareService::create([
        'pastor_id' => $this->pastor->id,
...
describe('CareServiceReportService', function (): void {
    it('collects all related appointments including parent and follow-ups', function (): void {
        $service = new CareServiceReportService($this->appointment);
...
describe('Report Generation API', function (): void {
    it('requires authentication', function (): void {
        $this->getJson("/api/care-service/appointments/{$this->appointment->uuid}/report")
            ->assertUnauthorized();
    });

    it('requires pastor role', function (): void {
        actingAs($this->member)
            ->get("/api/care-service/appointments/{$this->appointment->uuid}/report")
            ->assertForbidden()
            ->assertJson([
                'success' => false,
    ...
    it('returns 404 for non-existent appointment', function (): void {
        actingAs($this->pastor)
            ->get('/api/care-service/appointments/non-existent-uuid/report')
            ->assertNotFound()
            ->assertJson([
                'success' => false,
    ...
    it('returns 400 for unsupported format', function (): void {
        actingAs($this->pastor)
            ->get("/api/care-service/appointments/{$this->appointment->uuid}/report?format=invalid")
            ->assertBadRequest()
            ->assertJson([
                'success' => false,
    ...
    it('generates PDF report successfully', function (): void {
        $response = actingAs($this->pastor)
            ->get("/api/care-service/appointments/{$this->appointment->uuid}/report?format=pdf");
    ...
    it('generates Excel report successfully', function (): void {
        $response = actingAs($this->pastor)
            ->get("/api/care-service/appointments/{$this->appointment->uuid}/report?format=excel");
    ...
    it('generates Word report successfully', function (): void {
        $response = actingAs($this->pastor)
            ->get("/api/care-service/appointments/{$this->appointment->uuid}/report?format=word");
...
describe('Edge Cases', function (): void {
    it('handles appointment without parent', function (): void {
        $standaloneAppointment = CareService::create([
            'pastor_id' => $this->pastor->id,
        ...
        $service = new CareServiceReportService($standaloneAppointment);
        $data = $service->getReportData();
    ...
    it('handles appointment without follow-ups', function (): void {
        $noFollowUpsAppointment = CareService::create([
            'pastor_id' => $this->pastor->id,
        ...
        $service = new CareServiceReportService($noFollowUpsAppointment);
        $data = $service->getReportData();
    ...
    it('handles appointment without pastor notes', function (): void {
        $noNotesAppointment = CareService::create([
            'pastor_id' => $this->pastor->id,
        ...
        $service = new CareServiceReportService($noNotesAppointment);
        $data = $service->getReportData();
    ...
    it('handles appointment without client notes', function (): void {
        $noClientNotesAppointment = CareService::create([
            'pastor_id' => $this->pastor->id,
        ...
        $service = new CareServiceReportService($noClientNotesAppointment);
        $data = $service->getReportData();
    ...
    it('handles duration formatting for hours only', function (): void {
        $exactHourAppointment = CareService::create([
            'pastor_id' => $this->pastor->id,
        ...
        $service = new CareServiceReportService($exactHourAppointment);
        $data = $service->getReportData();
    ...
    it('handles duration formatting for minutes only', function (): void {
        $minutesOnlyAppointment = CareService::create([
            'pastor_id' => $this->pastor->id,
        ...
        $service = new CareServiceReportService($minutesOnlyAppointment);
        $data = $service->getReportData();
