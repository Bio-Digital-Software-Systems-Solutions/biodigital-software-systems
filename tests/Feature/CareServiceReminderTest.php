...
it('finds appointments within the 24-hour reminder window', function (): void {
    ...
    $this->artisan('care-service:send-reminders')
        ->assertSuccessful();

    Mail::assertQueued(CareServiceAppointmentReminder::class, fn ($mail) => $mail->hasTo($appointment->client_email));

    Mail::assertQueued(CareServicePastorReminder::class, fn ($mail) => $mail->hasTo($this->pastor->email));
...
it('does not send reminders for appointments already reminded', function (): void {
    ...
    Mail::assertNotQueued(CareServiceAppointmentReminder::class);
    Mail::assertNotQueued(CareServicePastorReminder::class);
});
...
it('sends reminders for pending appointments', function (): void {
    ...
    Mail::assertQueued(CareServiceAppointmentReminder::class);
    Mail::assertQueued(CareServicePastorReminder::class);
...
it('does not send reminders in dry run mode', function (): void {
    ...
    $this->artisan('care-service:send-reminders --dry-run')
        ->assertSuccessful();
...
it('respects custom hours option for reminder window', function (): void {
    ...
    $this->artisan('care-service:send-reminders --hours=48')
        ->assertSuccessful();
...
it('handles multiple appointments in the reminder window', function (): void {
    ...
    Mail::assertQueued(CareServiceAppointmentReminder::class, 3);
    // And to pastor for each appointment
    Mail::assertQueued(CareServicePastorReminder::class, 3);
});
...
it('sends SMS reminder when SMS is enabled', function (): void {
    ...
    config([
        'care_service.integrations.sms.enabled' => true,
    ...
    $service = new CareServiceNotificationService;
...
it('does not send SMS when SMS is disabled', function (): void {
    ...
    config([
        'care_service.integrations.sms.enabled' => false,
    ]);
...
it('sends WhatsApp reminder when WhatsApp is enabled', function (): void {
    ...
    config([
        'care_service.integrations.whatsapp.enabled' => true,
        'services.twilio.sid' => 'test_sid',
        'services.twilio.token' => 'test_token',
...
it('does not send WhatsApp when WhatsApp is disabled', function (): void {
    ...
    config([
        'care_service.integrations.whatsapp.enabled' => false,
    ]);
...
it('client reminder email contains appointment details', function (): void {
    ...
    $mail = new CareServiceAppointmentReminder($appointment);
...
it('pastor reminder email contains client information', function (): void {
    ...
    $mail = new CareServicePastorReminder($appointment);
    $rendered = $mail->render();
...
it('respects disabled reminders configuration', function (): void {
    ...
    config(['care_service.notifications.reminders.enabled' => false]);
    ...
    $this->artisan('care-service:send-reminders')
        ->assertSuccessful()
        ->expectsOutput('Care service reminders are disabled in configuration.');
...
it('respects disabled client reminder configuration', function (): void {
    ...
    config(['care_service.notifications.reminders.send_client_reminder' => false]);
...
it('respects disabled pastor reminder configuration', function (): void {
    ...
    config(['care_service.notifications.reminders.send_pastor_reminder' => false]);
