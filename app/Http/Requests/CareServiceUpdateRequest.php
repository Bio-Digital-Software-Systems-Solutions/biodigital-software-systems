...
class CareServiceUpdateRequest extends FormRequest
{
    /**
    ...
    {
        // Check if user has permission to update care service appointments
        if (! auth()->check() || ! auth()->user()->can('care_service.edit')) {
            return false;
        }

        // Get the care service appointment being updated
        $careService = $this->route('careService');

        // Only pastors can update their own appointments
        if ($careService && auth()->user()->hasRole('pastor')) {
            return $careService->pastor_id === auth()->id();
        }

        // Allow admins to update any appointment
        return auth()->user()->can('care_service.manage_all');
    }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'status' => [
                ...
                function ($attribute, $value, $fail) use ($careService): void {
                    if ($careService) {
                        // Validate status transitions
                        $currentStatus = $careService->status;
    ...
    protected function validateTimeSlotAvailability($validator): void
    {
        ...
        $pastorId = $careService->pastor_id;
        $appointmentDate = $this->input('appointment_date', $careService->appointment_date->format('Y-m-d'));
        $appointmentTime = $this->input('appointment_time', $careService->appointment_time->format('H:i'));
        $duration = $this->input('duration_minutes', $careService->duration_minutes);
        ...
        if (! CareService::isTimeSlotAvailable($pastorId, $appointmentDateTime, $duration, $careService->id)) {
    ...
    protected function validateAppointmentIsEditable($validator): void
    {
        ...
        if (in_array($careService->status, $restrictedStatuses) && (! auth()->user()->can('care_service.manage_all') || $this->except(['status', 'cancellation_reason']))) {
            $validator->errors()->add(
                'status',
        ...
        if ($careService->appointment_time < now() && $this->except(['status', 'notes'])) {
            $validator->errors()->add(
                'appointment_time',
