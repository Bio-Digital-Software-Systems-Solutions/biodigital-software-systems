...
class CareServiceNotificationService
{
    /**
    ...
    {
        return config('care_service.integrations.sms.enabled', false)
    ...
    {
        return config('care_service.integrations.whatsapp.enabled', false)
            && ! empty(config('services.twilio.sid'))
            && ! empty(config('services.twilio.token'))
     ...
     */
    public function sendSmsReminder(CareService $appointment): bool
    {
     ...
     */
    public function sendWhatsAppReminder(CareService $appointment): bool
    {
     ...
     */
    public function sendSmsPastorReminder(CareService $appointment): bool
    {
     ...
     */
    protected function buildSmsReminderMessage(CareService $appointment): string
    {
        $date = $appointment->appointment_date->format('d/m/Y');
        $time = $appointment->appointment_time->format('H:i');
        $pastor = $appointment->pastor->first_name.' '.$appointment->pastor->last_name;
        $church = config('care_service.church_name', config('app.name'));
     ...
     */
    protected function buildWhatsAppReminderMessage(CareService $appointment): string
    {
        $date = $appointment->appointment_date->format('d/m/Y');
        $time = $appointment->appointment_time->format('H:i');
        $pastor = $appointment->pastor->first_name.' '.$appointment->pastor->last_name;
        $church = config('care_service.church_name', config('app.name'));
        $phone = config('care_service.church_phone', '+49 89 123456789');
     ...
     */
    protected function buildSmsPastorReminderMessage(CareService $appointment): string
    {
     ...
     */
    public function sendConfirmationNotification(CareService $appointment, string $channel = 'all'): array
    {
        $results = [
            'sms' => false,
            'whatsapp' => false,
     ...
     */
    protected function buildConfirmationMessage(CareService $appointment): string
    {
        $date = $appointment->appointment_date->format('d/m/Y');
        $time = $appointment->appointment_time->format('H:i');
        $church = config('care_service.church_name', config('app.name'));
     ...
     */
    protected function buildWhatsAppConfirmationMessage(CareService $appointment): string
    {
        $date = $appointment->appointment_date->format('d/m/Y');
        $time = $appointment->appointment_time->format('H:i');
        $pastor = $appointment->pastor->first_name.' '.$appointment->pastor->last_name;
     ...
     */
    public function sendCancellationNotification(CareService $appointment, string $channel = 'all'): array
    {
        $results = [
            'sms' => false,
            'whatsapp' => false,
     ...
     */
    protected function buildCancellationMessage(CareService $appointment): string
    {
        $date = $appointment->appointment_date->format('d/m/Y');
        $time = $appointment->appointment_time->format('H:i');
        $church = config('care_service.church_name', config('app.name'));
