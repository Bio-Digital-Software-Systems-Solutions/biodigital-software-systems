...
export default function ProjectBoard({ project, tasksByStatus, users, sprints, filters }: Props) {
    ...
    const handleFilter = (key: string, value: string) => {
        ...
        router.get(route('projects.board', project.uuid), newFilters, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const clearFilters = () => {
        setLocalFilters({});
        router.get(route('projects.board', project.uuid), {}, {
            preserveState: true,
            preserveScroll: true,
        });
    };
                    ...
                    {Object.entries(statusConfig).map(([status, config]) => (
                        ...
                        >
                            ...
                            <div className={`${config.color} rounded-b-lg p-2 space-y-2 min-h-[500px] flex-1`}>
                                ...
                                {(!tasksByStatus[status as keyof typeof tasksByStatus] || tasksByStatus[status as keyof typeof tasksByStatus].length === 0) && (
