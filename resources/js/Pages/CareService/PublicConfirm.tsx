...
export default function PublicConfirm({ appointment }: Props) {
    ...
    const handleConfirm = async () => {
        ...
        try {
            const response = await apiFetch<{ success: boolean; message: string }>(`/api/care-service/appointments/${appointment.uuid}/confirm`, {
                method: 'POST',
            });

            if (response.success && response.data?.success) {
                toast.success(response.data.message || 'Rendez-vous confirmé avec succès !');
                router.visit('/care-service/success');
            } else {
    ...
    const handleCancel = () => {
        router.visit(`/care-service/cancel/${appointment.uuid}`);
    };
    ...
    return (
        <>
            <Head title="Confirmer votre rendez-vous - Care Services" />
                            ...
                            <CardContent className="pt-6">
                                <div className="text-center mb-6">
                                    ...
                                    <p className="text-gray-600 dark:text-gray-400">
                                        Veuillez confirmer ou annuler votre rendez-vous de care service.
                                    </p>
                                </div>
