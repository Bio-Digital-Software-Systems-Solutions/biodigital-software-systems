...
export default function DepartmentDocuments({
        ...
        return tree
            .map(yearData => {
                const filteredMonths = yearData.months
                    .map(monthData => ({
                        ...monthData,
                        documents: monthData.documents.filter(doc =>
                            documentMatchesSearch(doc, debouncedSearchTerm)
                        ),
                    }))
                    .filter(monthData => monthData.documents.length > 0);

                return {
                    ...yearData,
                    months: filteredMonths,
                    document_count: filteredMonths.reduce((acc, m) => acc + m.documents.length, 0),
                };
            })
    ...
    useEffect(() => {
        if (debouncedSearchTerm) {
            // Expand all years and months that have matching documents
                                            ...
                                            return (
                                                <div key={monthKey} className="border-l pl-4 mt-2">
                                                    ...
                                                    >
                                                        ...
                                                        <span className="text-sm text-muted-foreground">
                                                            ({monthData.documents.length} document{monthData.documents.length > 1 ? 's' : ''})
                                                        </span>
                                                    </button>
                ...
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="space-y-4 py-4 px-3">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                <Label htmlFor="upload-category">Catégorie (optionnel)</Label>
                                <Input
                                    id="upload-category"
                                    placeholder="Ex: Procès-verbal, Budget, Rapport..."
